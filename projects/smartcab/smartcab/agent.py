...
class LearningAgent(Agent):
    ...
    def build_state(self):
        ...
        # Creating a new state is implemented in createQ
        light = inputs['light']
        left = inputs['left']
        right = inputs['right']
    ...
    def get_maxQ(self, state):
        ...
        actions_values = self.Q[state]


        maxQ = max(actions_values.values())
    ...
    def createQ(self, state):
        ...
        if self.learning and state in self.Q:
            pass
        elif self.learning and not state in self.Q:
    ...
    def choose_action(self, state):
        ...
        if self.learning == True :
            ...
            flag = True # Checks if every value of the state is 0
            for x in self.Q[state].values():
                if x != 0:
                    flag =False
            if flag:
                action = None      # If every Q-value for a state is 0 then take no action.
            else:
                action_values = self.Q[state]
                for action in action_values:
                    if action_values[action] == maxQ:
                        maxAction = action
                action = maxAction if self.epsilon < random.random() else self.valid_actions[random.randint(0, 3)]
    ...
    def learn(self, state, action, reward):
        ...
        self.Q[state][action] += (self.alpha*(learned_value - self.Q[state][action]))
        return None
...
def run():
    ...
    agent = env.create_agent(LearningAgent,learning = True, alpha = 0.85,epsilon=1)
